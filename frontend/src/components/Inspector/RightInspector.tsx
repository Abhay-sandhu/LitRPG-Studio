...
export const RightInspector: React.FC<RightInspectorProps> = ({
  ...
  if (collapsed) {
    return (
      <aside className="w-12 border-l border-slate-800/80 bg-slate-900/60 flex flex-col items-center py-3 select-none transition-all">
        <button
          type="button"
  ...
  return (
    <aside className="w-80 border-l border-slate-800/80 bg-slate-900/60 flex flex-col select-none transition-all text-sm">
      {/* Header & Tabs */}
      <div className="p-3 border-b border-slate-800/60 flex items-center justify-between">
        <button
          type="button"
          onClick={onToggleCollapse}
          className="p-1.5 text-slate-400 hover:text-slate-200 hover:bg-slate-800 rounded transition-colors mr-2"
          title="Collapse Inspector"
        >
          <ChevronRight className="w-4 h-4" />
        </button>

        <div className="flex bg-slate-950/70 p-0.5 rounded-lg border border-slate-800/80 w-full">
          <button
            type="button"
          ...
          >
            <span>AI Drafts</span>
            {drafts.length > 0 && (
              <span className="w-4 h-4 rounded-full bg-amber-500/30 text-amber-300 text-[10px] font-bold flex items-center justify-center border border-amber-500/50">
                {drafts.length}
              </span>
            )}
          </button>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {activeTab === 'sheet' ? (
          <>
            {/* Identity Card */}
            <div className="bg-slate-950/70 border border-slate-800/90 rounded-xl p-3.5 space-y-2">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-bold text-slate-100">{character.name}</h3>
                  <p className="text-xs text-slate-400">Class: {character.className}</p>
                </div>
                <div className="text-right">
                  <span className="text-xs font-bold text-sky-400 bg-sky-950/60 border border-sky-800/50 px-2 py-0.5 rounded-full">
              ...
              <div className="space-y-1.5 pt-2">
                <div>
                  <div className="flex justify-between text-[11px] font-medium text-rose-400 mb-0.5">
                ...
                <div>
                  <div className="flex justify-between text-[11px] font-medium text-sky-400 mb-0.5">
                    <span className="flex items-center gap-1">
            ...
            <div className="bg-slate-950/70 border border-slate-800/90 rounded-xl p-3.5 space-y-2.5">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="bg-slate-900/90 border border-slate-800 p-2 rounded-lg">
                  ...
                  <div className="text-base font-bold text-slate-100">{character.stats.str}</div>
                </div>
                <div className="bg-slate-900/90 border border-slate-800 p-2 rounded-lg">
                  <span className="text-slate-400">AGI (Agility)</span>
                  <div className="text-base font-bold text-slate-100">{character.stats.agi}</div>
                </div>
                <div className="bg-slate-900/90 border border-slate-800 p-2 rounded-lg">
                  <span className="text-slate-400">INT (Intelligence)</span>
                  <div className="text-base font-bold text-slate-100">{character.stats.int}</div>
                </div>
                <div className="bg-slate-900/90 border border-slate-800 p-2 rounded-lg">
                  <span className="text-slate-400">VIT (Vitality)</span>
                  <div className="text-base font-bold text-slate-100">{character.stats.vit}</div>
                </div>
              </div>

              <div className="pt-1 text-[11px] text-slate-500 font-mono bg-slate-900/50 p-2 rounded border border-slate-800/50">
            ...
            <div className="bg-slate-950/70 border border-slate-800/90 rounded-xl p-3.5 space-y-2">
              ...
              <div className="space-y-1.5">
                {character.inventory.map((item) => (
                  <div
                    key={item.id}
                    className="flex items-center justify-between px-2.5 py-1.5 rounded-lg bg-slate-900/80 border border-slate-800 text-xs text-slate-200"
                  >
                    <span>{item.name}</span>
          ...
          <>
            ...
            <div className="space-y-3">
              <div className="text-xs text-slate-400 flex items-center justify-between">
                ...
                <span className="text-[11px] text-amber-400">Needs Review</span>
              </div>
              ...
              {drafts.length === 0 ? (
                ...
                drafts.map((draft) => (
                  <div
                    key={draft.id}
                    className="bg-slate-950 border border-amber-500/30 rounded-xl p-3 space-y-2 shadow-sm hover:border-amber-500/50 transition-colors"
                  >
                    <div className="flex items-center justify-between">
                    ...
                    <p className="text-xs font-medium text-slate-200">{draft.desc}</p>
                    ...
                    <div className="flex items-center gap-2 pt-1 border-t border-slate-800/60">
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-1 py-1 px-2 rounded-md bg-emerald-950/60 text-emerald-300 border border-emerald-800/50 hover:bg-emerald-900/60 text-xs font-medium transition-colors"
                      >
                      ...
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-1 py-1 px-2 rounded-md bg-slate-900 text-slate-400 border border-slate-800 hover:bg-slate-800 hover:text-slate-200 text-xs font-medium transition-colors"
                      >
